...
main:
  ...
  beq $t3, $t1, jumpA # t3 /= t1 does not jump
  nop
  nop
  add $t3, $t3, $t3 # en r11 un 6 = 3 + 3
  beq $t1, $t1, jumpB #dependencia con la 2� anterior # en r10 un 15 = 7 + 8
  jumpA: add $t5, $t1, $t2  # en r14 un 3 = 1 + 2
  nop
  nop
  jumpB: add $t2, $t3, $t5 #dependencia con la 3� anterior  # en r10 un 22 = 6 + 16
  nop
  nop
  nop
  lw $t3, 0($zero) # en r9 un 1
  beq $t3, $t5, jumpC # t3 /= t5 does not jump
  nop
  nop
  nop
  lw $t3, 4($zero) # en r9 un 2
  beq $t3, $t3, jumpD # t3 = t3 jump
  jumpC: lw $t3, 8($zero) # en r9 un 4
  nop
  nop
  jumpD: add $t4, $t2, $t3 # dependencia con la 3� anterior # en r12 22 = 20 + 2
  nop
  nop
  nop
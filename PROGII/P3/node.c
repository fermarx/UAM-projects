...
void* node_copy (const void *src){
    ...
    trg->predecessorid = node->predecessorid;
...
int node_print (FILE *pf, const void *n){
    ...
    return fprintf(pf, "[%s, %li, %li, %i, %i]", node->name, node->id, node->predecessorid, node->label, node->nConnect);
}
...
Status node_setPredecessorId (Node *n, long predid){
    ...
    if(!n || !predid) return ERROR;

    n->predecessorid = predid;
    if (!n->predecessorid) return ERROR;

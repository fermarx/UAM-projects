...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    ethUP = struct.unpack('h', eth)
    if ethUP in upperProtos:
        upperProtos[ethUP](us, header, data[14:], org)
    return
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    ethUP = struct.unpack('h', ethertype)
    upperProtos[ethUP](callback_func)
...
def startEthernetLevel(interface:str) -> int:
    ...
    handle = pcap_open_live(interface, ETH_FRAME_MAX, PROMISC, TO_MS, err)
...
def sendEthernetFrame(data:bytes,len:int,etherType:int,dstMac:bytes) -> int:
    ...
    frame = bytearray()
    #Use extend to append to frame
    frame.extend(dstMac)
    frame.extend(macAddress)
    frame.extend(bytes(etherType))
    frame.extend(data)
    framelen = len + 14
    ...
    if framelen < ETH_FRAME_MIN:
        frame.extend(bytes([0])*(ETH_FRAME_MIN-framelen))
        framelen = ETH_FRAME_MIN

    if pcap_inject(handle, bytes(frame), framelen) == -1:
        logging.info('Error en pcap_inject.')
        return -1

...
def processARPRequest(data:bytes,MAC:bytes)->None:
    ...
    '''
    global myIP
    #mac origen en la cabecera de arp
    macorigen = data[8:14]
    ...
    #arp reply con la ip origen 
    arp = createARPReply(IPorg, macorigen)
    arplen = len(arp)
    sendEthernetFrame(arp, arplen, int(data[2:4]), macorigen)
...
def createARPRequest(ip:int) -> bytes:
    ...
    arp = bytearray()
    #arp header
    arp.extend(ARPHeader)
    #0x0001 es para el response, opcode
    arp.extend(bytes([0x00,0x01]))
    #var global, sender eth
    arp.extend(myMAC)
    #var global packeada, sender ip 
    arp.extend(struct.pack('!I', myIP))
    #6 0s, target eth
    arp.extend(bytes([0]*6))
    #argumento packeade, target ip    
    arp.extend(struct.pack('!I', ip))
    return arp
...
def createARPReply(IP:int ,MAC:bytes) -> bytes:
    ...
    '''
    global myMAC,myIP
    #mismo que en el e antes
    arp = bytearray()

    arp.extend(ARPHeader)
    arp.extend(bytes([0x00,0x02]))
    arp.extend(myMAC)
    arp.extend(struct.pack('!I', myIP))
    arp.extend(MAC)
    #target ip ya viene packeado    
    arp.extend(IP)
    return arp
...
def process_arp_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes,srcMac:bytes) -> None:
    '''
        ...
        Retorno: Ninguno
    '''
    #  HW type es 0x0001 en caso de eth dir,Protocol type es 0x800 en dirs ip,   HW size es 6(en bytes),        Protocol size es 4 en bytes
    if data[0:2] == bytes([0x00, 0x01]) and data[2:4] == bytes([0x80, 0x00]) and data[4:5] == bytes([0x06]) and data[5:6] == bytes([0x04]):
    	#opcode == 0x0001 o 0x0002 para req o rep
	    opcode = data[6:8]
...
def ARPResolution(ip:int) -> bytes:
    ...
    for i in range(3):
        sendEthernetFrame(trama,len(trama),bytes([0x08, 0x06]),broadcastAddr)
